...
function pack()
{
    try
    {
        var savePath = rootdir + "/" + "src/autofiles/ShaderConfig.ts";
        var shaders = readFiles(getFilePaths(rootdir + "/" + "shaders", null, 1));
        var shadermodules = readFiles(getFilePaths(rootdir + "/" + "shaders/modules", null, 1));
...
function watchPack()
{
    ...
    fs.watch(rootdir + "/shaders", { recursive: true }, changeHandler);
